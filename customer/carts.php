    ...
    <div class="row g-4 align-items-start pb-5">
                        ...
                        <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between gap-3 gap-sm-4 py-3 border-bottom border-light">
                            ...
                            <div class="d-flex align-items-center gap-3 gap-sm-4 flex-grow-1 min-w-0">
                                <div class="bg-light rounded-4 d-flex align-items-center justify-content-center border" style="width: 80px; height: 80px; flex-shrink:0; overflow:hidden;">
                                    <img :class="item.image ? 'd-block' : 'd-none'" :src="item.image" @error="item.image = ''" alt="" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                    <i :class="!item.image ? 'd-block' : 'd-none'" class="bi bi-journal-bookmark-fill text-secondary opacity-50 fs-4"></i>
                                </div>
                                <div class="min-w-0">
        ...
        <div class="col-12 col-lg-4">
            <div class="card-custom bg-white border-0 shadow-sm p-4 p-sm-5">
                ...
                <div class="pt-2 mb-4">
                    <h6 class="fw-bold text-dark mb-3">Customer Details</h6>
                    <div class="d-flex flex-column gap-3">
                        <div>
                            <label class="form-label text-secondary small fw-semibold mb-1">Full Name</label>
                            <input type="text" x-model.trim="checkoutForm.name" class="form-control" placeholder="Enter full name" required>
                        </div>
                        <div>
                            <label class="form-label text-secondary small fw-semibold mb-1">Email</label>
                            <input type="email" x-model.trim="checkoutForm.email" class="form-control" placeholder="Enter email address" required>
                        </div>
                        <div>
                            <label class="form-label text-secondary small fw-semibold mb-1">Delivery / Pickup Address</label>
                            <textarea x-model.trim="checkoutForm.address" class="form-control" rows="3" placeholder="Enter complete address" required></textarea>
                        </div>
                        <div>
                            <label class="form-label text-secondary small fw-semibold mb-1">Payment Method</label>
                            <select x-model="checkoutForm.payment_method" class="form-select" required>
