...
<div :class="tab === 'shop' ? 'd-flex' : 'd-none'" x-cloak class="container-fluid px-0 h-100 flex-column">
    <header class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-4">
        ...
        <div class="position-relative" style="max-width:320px; width: 100%;">
            ...
            <input type="text" x-model="search" @input="productPage = 1" placeholder="Search products..." class="form-control bg-white shadow-sm border-0" style="padding-left: 40px; padding-top:10px; padding-bottom:10px; border-radius:1rem;">
        </div>
    </header>
            ...
            <div class="col-12 col-sm-6 col-lg-4 col-xl-3">
                <div class="card h-100 card-custom border-0 bg-white" style="cursor: pointer; transition: transform 0.2s, box-shadow 0.2s;" onmouseover="this.style.transform='translateY(-4px)'; this.classList.add('shadow');" onmouseout="this.style.transform='none'; this.classList.remove('shadow');">
                    ...
                    <div class="bg-light d-flex align-items-center justify-content-center p-4 border-bottom" style="height: 180px; border-top-left-radius: 1rem; border-top-right-radius: 1rem; overflow:hidden;" @click="openModal(product)">
                        <img :class="product.image ? 'd-block' : 'd-none'" :src="product.image" @error="product.image = ''" alt="" style="max-width:100%; max-height:100%; object-fit:contain;">
                        <div :class="!product.image ? 'd-flex' : 'd-none'" class="flex-column align-items-center justify-content-center gap-2">
                            <i class="bi bi-journal-bookmark-fill" style="font-size: 48px; color: #d1d5db;"></i>
                            <span class="text-secondary fw-medium" style="font-size: 10px; letter-spacing: 0.5px;">NO IMAGE</span>
                        </div>
                    </div>
                    <!-- Body -->
    ...
    <div :class="filteredProducts.length > 0 ? 'd-flex' : 'd-none'" class="align-items-center justify-content-between border-top pt-3 mt-2">
        <span class="text-secondary small">
            Showing <span x-text="productRangeStart"></span>-<span x-text="productRangeEnd"></span> of <span x-text="filteredProducts.length"></span> products
        </span>
        <div class="d-flex align-items-center gap-2">
            <button class="btn btn-sm btn-outline-secondary" @click="prevProductPage" :disabled="productPage === 1">Previous</button>
            <span class="badge bg-dark" x-text="productPage"></span>
            <button class="btn btn-sm btn-outline-secondary" @click="nextProductPage" :disabled="productPage >= totalProductPages">Next</button>
        </div>
    </div>
</div>
